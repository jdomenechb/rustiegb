...
use crate::audiold::noise::NoiseDescription;
use crate::audiold::pulse::PulseDescription;
...
use crate::audiold::wave::WaveDescription;
...
impl CpalAudioUnitOutput {
    ...
    pub fn update(&mut self, io_registers: Arc<RwLock<IORegisters>>) {
        ...
        if self.pulse_description_1.read().stop {
            io_registers.apu.nr52.set_ro_channel_flag_inactive(1);
        }

        {
            if self.pulse_description_2.read().stop {
                io_registers.apu.nr52.set_ro_channel_flag_inactive(2);
            }
        }

        {
            if self.wave_description.read().stop {
                io_registers.apu.nr52.set_ro_channel_flag_inactive(3);
            }
        }

        {
            if self.noise_description.read().stop {
                io_registers.apu.nr52.set_ro_channel_flag_inactive(4);
            }
        }
    }
