...
impl Memory {
    ...
    fn write_sweep_register(&self, position: Word, value: Byte) {
        println!("{:X}", position);
        println!("------");

        let sweep_time = value >> 4;
        println!("Sweep Time: {} ", sweep_time & 0b111);

        if sweep_time > 0 {
            let sweep_inc_dec = (value >> 3) & 0b1;

            if sweep_inc_dec == 0 {
                println!("Sweep Addition (Inc)");
            } else {
                println!("Sweep Substraction (Dec)");
            }

            println!("Number of sweep shift: {}", value & 0b111);
        }
    ...
    fn write_envelope_register(&self, position: Word, value: Byte) {
        println!("{:X}", position);
        println!("------");

        println!("Initial volume of envelope: {:X}", value >> 4);

        let envelope_dir = value >> 3 & 0b1;

        if envelope_dir == 0 {
            println!("Envelope Dir Attenuate");
        } else {
            println!("Envelope Dir Amplify");
        }

        println!("Number of envelope sweep: {}", value & 0b111);
    ...
    fn write_freq_hi_register(&self, position: Word, value: Byte) {
        println!("{:X}", position);
        println!("------");

        println!("Initial  {:X}", value >> 7);
        println!("Counter/consecutive selection  {:X}", (value >> 6) & 0x1);

        println!("Frequency's higher 3 bit: {}", value & 0b111);
